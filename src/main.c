...
*/
void calculate_light(t_rtv *rtv, t_light *lights, t_obj3d *objects)
{
	int			j;
	t_light		cur_light;
	t_vec3d		dist;
	...
	while(j++ < 3)
	{
		...
		if (!(calculate_shadows(objects, &light_ray, &t2)))
		{
...
void ray_trace(t_rtv *rtv)
{
	...
	{
		rtv->calc->cur_obj = -1;
		if (object_intersect(objects, &rtv->scene->ray, &rtv->calc->cur_obj, &rtv->calc->new_start) == false)
			break;
		if (normal_of_intersect(&rtv->calc->intersect_normal, &rtv->calc->new_start, objects, rtv->calc->cur_obj) == false)
			break;
		rtv->calc->cur_mat = materials[objects[rtv->calc->cur_obj].material];
		calculate_light(rtv, lights, objects);
